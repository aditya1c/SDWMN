...
class rolechanger(oldsimple_switch_13.SimpleSwitch13):
	...
	def __init__(self, *args, **kwargs):
		...
		# self.monitor_thread = hub.spawn(self._timeoutmonitor)
		self.senttime = {}
	...
	def _echomonitor(self):
		while True:
			...
			time.sleep(5) # echo check

	# def _timeoutmonitor(self):
	# 	while True:
	# 		for items in self.datapaths:
	# 			if time.time() - self.senttime[items] >= 4.98:
	# 				if time.time() - self.senttime[items] <= 5:
	# 					self.tooktime[items] = 10
	# 					print "Not Received from ", items
	# 					fw_data = open(str(items),'a')
	# 					fw_data.write('%16.6f' % self.senttime[items])
	# 					fw_data.write(':')
	# 					fw_data.write('%7.6f' % self.tooktime[items])
	# 					fw_data.write('\n')
	# 					self.senttime[items] += 5
	# 					fw_data.close()
	# 		time.sleep(0.1)
